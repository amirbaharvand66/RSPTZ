function [N__xx, N__xy, N__yy, M__xx, M__xy, M__yy, Q__x, Q__y, lam] = result_f_m(scf, x, y, z, Z, lam, theta, k, tre_elas)
...
if strcmpi (tre_elas, 'on')
    [sigma__zz, tau__yz, tau__xz] = three_d_elasticity_eqn(x, y, z, Z, lam, theta, k);
...
Q__x = scf * int(tau__xz, z, [Z(k), Z(k + 1)]);
Q__y = scf * int(tau__yz, z, [Z(k), Z(k + 1)]);